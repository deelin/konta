...
def handle_ref(cell_args, fullpath):
    ref = cell_args[0]
    col_num = letter_to_num(ref[0])
    row_num = int(ref[1:])

    ref_row = read_row(row_num, fullpath)
...
def eval_spreadsheet(fullpath):
    if os.path.isdir(fullpath):
        ...
        for i, row in enumerate(reader):
            ...
            for cell in row:
                if last_cell != None:
                    sys.stdout.write(last_cell + ',\t')
                last_cell = eval_cell(cell, fullpath)
            sys.stdout.write(last_cell + '\n')
        f.close()
